      ...
      <View style={styles.navTabs}>
        {tabs.map(t => (
          <TouchableOpacity
            ...
            onPress={t.onPress}
          >
            <Ionicons name={t.icon} size={15} color={t.id === 'movies' ? '#e8f4ff' : '#5a7d9a'} style={{ marginRight: 6 }} />
    ...
    <TouchableOpacity style={[styles.card, { width: cardWidth }]} onPress={onPress} activeOpacity={0.8}>
      {posterUri && !imgFailed ? (
        ...
        <View style={[styles.poster, styles.posterFallback, { width: cardWidth, height: cardHeight }]}>
          <Ionicons name="film-outline" size={28} color="#1e3d5c" />
        </View>
      )}
      ...
      (async () => {
        try {
          const [cats, items] = await Promise.all([
            ...
            fetchVodItems({ limit: 1000 }).catch(() => []),
          ]);
          if (!alive) return;
